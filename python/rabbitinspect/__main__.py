...
def format_finding(file_path: str, finding: dict) -> str:
    return f'{file_path}:{finding["line"]}:{finding["col"]}: {finding["code"]} {finding["message"]}'
...
def _build_exclude_matcher(patterns: list[str]) -> Callable[[pathlib.Path], bool]:
    compiled = []
    for pat in patterns:
...
EXPLANATIONS: dict[str, str] = {
    ...
    'RAB018': 'Too many function parameters (over 6). Consider using a dataclass or splitting the function.',
    'RAB019': 'Use subprocess.run() instead of os.system() for better control, error handling, and security.',
    ...
    'RAB029': "Use 'x' or 'not x' instead of comparing to True/False with ==. Boolean values are already truthy/falsy.",
    ...
    'RAB037': 'Use a generator expression instead of map()/filter() with a lambda for better readability.',
    'RAB038': "Use 'x in {const, ...}' instead of 'x in [const, ...]' for O(1) membership.",
    'RAB039': 'Use native generics (list[...], dict[...]) instead of typing.List, typing.Dict (Python 3.9+).',
    ...
    'RAB044': 'Use X | None instead of Optional[X], and A | B instead of Union[A, B] (Python 3.10+).',
    ...
    'RAB063': "Use '==' instead of 'is' to compare with literals. 'is' is for identity, not value equality.",
    ...
    'RAB066': 'Function defined inside a loop is recreated on every iteration. Move it outside the loop.',
    'RAB067': "Variable/function shadows a Python built-in name. Rename to 'name_' to avoid confusion.",
    ...
    'RAB077': 'Assertion on a tuple literal is always true; drop the parentheses (assert cond, msg).',
    'RAB078': 'except Exception: pass silently swallows all exceptions. Log the error or handle it properly.',
    ...
    'RAB084': 'Bare raise outside an except block raises RuntimeError (no active exception to re-raise).',
    ...
    'RAB092': "Class attribute is missing a type annotation. Add ': Type' to document the expected type.",
    'RAB093': "Module-level variable is missing a type annotation. Add ': Type' for better documentation.",
    ...
    'RAB097': 'Debugging call (print/breakpoint/pdb.set_trace) left in production code. Remove before committing.',
    ...
    'RAB100': "Redundant 'elif' after return/raise/break/continue. The terminal statement makes the elif unreachable.",
    'RAB101': 'Cyclomatic complexity exceeds the threshold. Consider simplifying the function by splitting it up.',
    'RAB102': 'Cognitive complexity exceeds the threshold. The function is hard to understand — consider refactoring.',
    'RAB103': "Self-comparison (e.g., 'x == x') always evaluates to True (or False for '!='). This is likely a typo.",
    ...
    'RAB106': "Too broad 'except Exception:' catches too many errors. Catch only the specific exceptions you expect.",
    'RAB107': 'Comment contains TODO/FIXME/HACK/XXX marker. Address the issue before shipping.',
    'RAB108': '__all__ should contain only string literals. Variables in __all__ may not resolve correctly.',
    'RAB109': 'Class name should use CamelCase convention (PEP 8).',
    'RAB110': 'Function name should use snake_case convention (PEP 8).',
    'RAB111': 'Module-level constant assigned a literal value should use UPPER_CASE naming (PEP 8).',
    ...
    'RAB119': '__init__ in a subclass does not call super().__init__(). The parent class may not initialize properly.',
    ...
    'RAB126': 'Magic number literal. Assign the value to a named constant to clarify its meaning.',
